...
with_bc_data_add_mean2 = with_bc_data_add_mean %>% left_join(fungal_richness_site)
...
ggplot(data = with_bc_data_add_mean, aes(x = Tave, y = mean_env, fill = Origin, color = Origin, group = Origin)) + 
  geom_smooth(method = "lm", se = F, aes(linetype = Origin), size = 0.5, formula = y ~ x) +
...
# joint plot
Figure_3b_right + annotation_custom(grob=ggplotGrob(Figure_3b_left), ymin = -0.58, ymax = -0.15, xmin=18, xmax=21.5) -> Figure_3b; Figure_3b
