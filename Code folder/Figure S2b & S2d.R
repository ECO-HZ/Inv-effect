...
(0.7359604 - 0.6887433)/0.6887433 # increased 6.9%
...
ggplot()+
  ...
  geom_abline(intercept = -0.2830512, slope = 1.4804843, linetype = 1, linewidth = 0.5) +
  ...
  scale_y_continuous(labels = scales::label_comma(accuracy =0.01), limits = c(0.54,0.88)) + 
  ...
  labs(x = NULL, 
       ...
       tag = "b", title = c("Among co-occurring native species")) -> Figure_S2b; Figure_S2b
...
ggplot()+
  ...
  scale_y_continuous(labels = scales::label_comma(accuracy =0.01), limits = c(0.45,0.88)) + 
  theme_bw() + mytheme + 
  #theme(legend.position = c(0.12, 0.60),
  #      legend.title = element_text(size = 10, color = "black")) + 
  labs(x = "Mean pair-wise Bray–Curtis\ndissimilarities estimated in the greenhouse experiment", 
       y = "", title = c("Among co-occurring alien species"), tag = "d") -> Figure_S2d; Figure_S2d
